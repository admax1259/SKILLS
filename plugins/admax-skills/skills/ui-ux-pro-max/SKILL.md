---
name: ui-ux-pro-max
description: "Search local UI/UX guidance for interface design, accessibility, typography, layout, charts and framework-specific implementation. Use for designing or reviewing user interfaces and targeted interaction fixes; not pure backend or infrastructure work."
---
...
## Existing design systems and Hallmark

Read the project's existing design instructions, tokens, `design.md`, and any `design-system/.../MASTER.md` before selecting a direction. Extend the established system for new pages; do not generate a competing system automatically. Use this skill for local product, UX, accessibility, and stack-specific evidence. Hallmark can supply visual composition and review guidance when requested or useful; do not automatically run both complete workflows. Preserve one authoritative design system and explain any conflicting recommendations before applying them.

For Chinese requests, translate the specific search intent into a short English query for the bundled English data, then explain relevant findings in the user's language. Read-only reviews must not edit code or use `--persist`. Persist only when saving design decisions is within the requested work, always to an explicit project output directory. Do not write to the plugin directory. Guidance and search matches are not proof of accessibility compliance: validate the actual rendered interface and report verification limits.
...
First resolve the absolute directory containing this loaded SKILL.md and assign it to `UIUX_SKILL_DIR` in your shell (this is a variable you set, not a guaranteed engine environment variable). Locate the installed skill through the engine if its path is unavailable; do not guess a cache location.
...
```bash
python3 -B "$UIUX_SKILL_DIR/scripts/search.py" "<query>" --domain <domain>
```

Requires Python 3.10+ with the standard library only. On Windows, use `py -3 -B` and equivalent shell quoting. If Python is unavailable, report the missing prerequisite; do not silently install it. The `-B` flag avoids writing bytecode into the installed plugin.
...
1. **New or unsettled design system** → use `--design-system`.
...
```bash
python3 -B "$UIUX_SKILL_DIR/scripts/search.py" "<product_type> <industry> <keywords>" --design-system [-p "Project Name"]
```
...
**Example:**
```bash
python3 -B "$UIUX_SKILL_DIR/scripts/search.py" "beauty spa wellness service" --design-system -p "Serenity Spa"
```
...
```bash
python3 -B "$UIUX_SKILL_DIR/scripts/search.py" "<query>" --design-system --persist -p "Project Name" --output-dir "<project-root>"
```
...
```bash
python3 -B "$UIUX_SKILL_DIR/scripts/search.py" "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
**Example:**
```bash
python3 -B "$UIUX_SKILL_DIR/scripts/search.py" "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```

### Step 3: Supplement with Detailed Searches (as needed)

```bash
python3 -B "$UIUX_SKILL_DIR/scripts/search.py" "<keyword>" --domain <domain> [-n <max_results>]
```
...
```bash
python3 -B "$UIUX_SKILL_DIR/scripts/search.py" "<keyword>" --stack <stack>
```
...
python3 -B "$UIUX_SKILL_DIR/scripts/search.py" "AI search tool modern minimal" --design-system -p "AI Search"

# Step 3: supplement
python3 -B "$UIUX_SKILL_DIR/scripts/search.py" "keyboard focus modal" --domain ux

# Step 4: stack guidelines
python3 -B "$UIUX_SKILL_DIR/scripts/search.py" "suspense streaming bundle" --stack nextjs
```
