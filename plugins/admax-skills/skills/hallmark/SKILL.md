---
name: hallmark
description: "Anti-AI-slop design skill for greenfield pages, audits, redesigns, and design extraction from URLs or screenshots. Use when the user asks to build a new app or landing page, wants to redesign something, invokes Hallmark by name, or asks for a Hallmark design audit, redesign, or reference study. Do not route generic code audits or research here."
metadata:
...
Invoke through the host skill mechanism, e.g. `$hallmark` in Codex or `/admax-skills:hallmark` in Claude Code, followed by the desired verb. Bare `audit`, `redesign`, and `study` below are Hallmark modes, not general-purpose commands.

Use available read-only web tools for the WebFetch steps; preserve the source restrictions in `references/study.md`. If the host cannot return the required HTML/CSS, use an authorized screenshot or report the evidence gap. Browser rendering may support visual checks for your own output; never claim viewport, contrast, or interaction checks passed without performing them. Preview scores are pending until the artifact has been built and checked; label subjective scores as self-assessment.

Existing user instructions, brand requirements, accessibility, framework conventions, and authorized scope take precedence over stylistic defaults in this skill and its references. Reuse context and approvals already supplied; ask only for missing decisions that materially affect the result. An audit is read-only, including no preflight cache or history writes. During builds, keep `.hallmark/` metadata limited to design choices, without credentials or private source content; refresh cached findings when any recorded source changes, including CSS and design.md. Never overwrite an existing token file or design system blindly. Tool/vendor/pricing examples in references are upstream snapshots: verify current availability when using them, and use host-provided tools without requiring a specific paid vendor.

The theme token library is bundled at [assets/catalog-tokens.css](assets/catalog-tokens.css). Copy only the selected theme's needed tokens into the target project's existing token system. Other upstream demo links are optional online examples, pinned to the imported revision; they are not required installation files.
...
**Reuse the brief first.** If audience, use case, and tone are already clear, state the interpretation and proceed. Otherwise ask one concise bundled question about the missing fields. If the user delegated those choices, infer and state them. Do not ask for the same information twice. Audit and study derive their context from the target.
...
**Do not skip the inference disclosure.** The opt-out respects delegated choices, not an excuse for the skill to be opaque. If the user can't see what was inferred, they can't redirect when it's wrong.
...
The per-theme axis values live as comments at the top of each theme's tokens block in [`site/css/tokens.css`](assets/catalog-tokens.css). When in doubt, name your candidate theme out loud and identify its three axis values; if two of three match the previous output, redirect.
...
6. **Slop test** — `58 / 58 ✓` if all gates pass, or `N / 58 — fails: <gate numbers>` if any are open. Before Build, write `pending`. Replace it with evidenced results after Step 7; mark checks unavailable in the host as unverified.
...
5. **Branch on the user's response:**
   ...
   - **"Lock the DNA"** (or any other emission trigger phrase per `study.md` § Trigger phrases) → emit a portable `design.md` of the DNA per `study.md` § Emitting a `design.md` from `study`. **In URL mode, run the attestation step first** — ask whether the source is (a) user's own, (b) public reference for the user's brand, or (c) something else. (c) refuses emission; (a) and (b) write the file with a `## Provenance` block recording the answer. **Image mode can emit the requested abstract design analysis**; an attached screenshot does not establish ownership of its contents. The emitted file becomes the project's locked system; subsequent runs defer to it.
