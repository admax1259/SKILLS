...
The bundled template implements GitHub secret helpers only. For GitLab, use verified provider-specific helpers or manual instructions; do not run `gh` against a GitLab project. Treat the template as an authoring resource, not an installer. Review generated stages before execution, avoid logging secrets, validate dotenv escaping for the target parser, and never source untrusted `.env` values as shell code.
